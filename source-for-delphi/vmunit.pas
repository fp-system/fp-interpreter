...
uses SysUtils,//ansiqoutedstr,stringreplace,exception,tformatsettings
     typeunit;//ustring;
...
const normalset = '_.0123456789' +                    //ansistring?
                  ...
                  'ÅÆÐËŒØŠŸŽßþ' +
                  'åæðëœøšÿžµÞ';
      specialset = '()[]{}»«,;°'+qot1+qot2+sharp+prefix;// idy special? , ; special?; +compose?type...//;

var eproc: array [0..xlimit] of fproc;
    normal,special: charset;
...
begin inc(i);
      ...
      if (ch<>'') then begin
         if charinset(s[i],special) then inc(i)
         else if charinset(s[i],normal) then
              repeat inc(i);
                     if (i>length(s)) then quit:=true
                     else quit:=not(charinset(s[i],normal))
              until quit
         else repeat inc(i);
                     if (i>length(s)) then quit:=true
                     else quit:=( charinset(s[i],normal)
...
begin ecall:=0;
      repeat equit:=true;
         //auf ab hier zurückrücken (3 spaces);
             einf:=infix[efun];
             {repeat einf:=infix[efun];//vorbau, wenn einf>xlimit
...
procedure initcharset(var tab: charset; s: ansistring);//--> charinset
var i: cardinal;
begin tab:=[];
      //for i:=0 to $FF do tab[chr(i)]:=false;
      for i:=1 to length(s) do tab:=tab+[s[i]]//[s[i]]:=true
end;

procedure initvm(mc,ms: int64);
begin fpformatsettings:=TFormatSettings.Create;//(Locale);
      initapi(mc,ms);
